...
from helpers.skills import get_all_skills, sort_skills
...
def main():
    ...
    search_page = SearchPage(query='Flutter')
    search_page.open(search_page.url)
    parsed_data = parse_links(search_page.get_links())
    search_page.quit()
    skills = get_all_skills(parsed_data)
    sorted_skills = sort_skills(skills)

    write_results(parsed_data, sorted_skills)
